...
// New: max height for points (meters). UI controls this.
let maxPointHeight = VIEW_FRAME_RADIUS_M;
...
function setRuntimeStatus(nextStatus) {
    ...
    // When entering scanning, remove idle-pattern points to avoid ghost trails
    if(nextStatus === 'scanning') {
        for(let i = 0; i < pointsInitialized; i++) {
            if(pointIdleFlags[i] === 1) {
                pointLives[i] = 0;
                pointIdleFlags[i] = 0;
                const c = i * 3;
...
function updateMaxHeightUi() {
    if(!maxHeightInput) return;
    const parsed = Number(maxHeightInput.value);
    if(Number.isFinite(parsed)) {
        maxPointHeight = parsed;
    }
    if(maxHeightValue) {
        maxHeightValue.textContent = `${maxPointHeight.toFixed(1)} m`;
    }
}
...
function addLidarPointFromRadians(dist, t, p, idlePoint) {
    ...
    // If point exceeds max height, record it as empty (so it won't render) and advance index
    if(positions[i + 1] > maxPointHeight) {
        pointLives[idx] = 0;
        pointIdleFlags[idx] = 0;
        colors[i] = 0; colors[i + 1] = 0; colors[i + 2] = 0;
        pointWriteIndex = (pointWriteIndex + 1) % MAX_POINTS;
        if(pointsInitialized < MAX_POINTS) pointsInitialized++;
...
function decayPointCloud(dtSec) {
    ...
    for(let idx = 0; idx < pointsInitialized; idx++) {
        ...
        // hide points that exceed the configured max height
        const py = positions[idx * 3 + 1];
        if(py > maxPointHeight) {
            life = 0;
        }
...
window.addEventListener('load', () => {
    ...
    // initialize max-height UI
    if(maxHeightInput && maxHeightInput.value) {
        maxPointHeight = Number(maxHeightInput.value);
    }
    updateMaxHeightUi();
...
// wire max-height input after DOM is ready (non-blocking)
if(maxHeightInput) {
